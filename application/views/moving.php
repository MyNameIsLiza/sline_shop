...
<div class="jumbotron">
    <form id="add_form" class="needs-validation">
        <h2 class="sub-header">Переміщення</h2>
        ...
        <div class="input-group mb-3">
            ...
            <select id="places" name="new_place_id" class="form-control" size="1">
                <?php foreach ($places as $place): ?>
                    <option value="<?= $place['place_id'] ?>" <? if ($place['place_id'] == $this->user_info['place_id']) echo 'disabled' ?>><?= $place['name'] ?></option>
                <?php endforeach; ?>
            </select>
        </div>
        ...
        <button type="submit" class="btn btn-success" <!--onclick="edit_type(this)-->">Перемістити</button>
    </form>
</div>
<script>
    ...
    $('#add_form').submit(function (event) {
        ...
        console.log(data);
        $.post("storage/move_good", data, function (result) {
            console.log(result);
            if (result) {
                console.log("Victory");
                document.location.reload();
                alert("Ви успішно перемістили товар");
            }
        }, "json");
